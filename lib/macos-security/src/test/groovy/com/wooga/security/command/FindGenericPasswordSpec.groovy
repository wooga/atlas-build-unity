...
import static com.wooga.security.SecurityHelper.addGenericPassword

@Requires({ os.macOs })
class FindGenericPasswordSpec extends FindPasswordSpec<FindGenericPassword> {
    FindGenericPassword command

    def setup() {
        command = new FindGenericPassword(testAccount, testServiceOrServer)
        command.withKeychain(testKeychainLocation)
    }

    @Override
    void addTestPassword(String account, String serviceOrServer, String password) {
        addGenericPassword(testKeychainLocation, testAccount, testServiceOrServer, testPassword)
    }
