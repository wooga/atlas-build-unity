...
@Requires({ os.macOs })
class FindKeySpec extends SecurityCommandSpec<FindKey> {
    FindKey command = new FindKey()

    def "finds key in keychain"() {
        ...
        command.withKeychain(testKeychainLocation)
                .withType(FindKey.Type.Private)
                .withLabel(privateKeyName)
        ...
        result != null
        !result.empty
        result.contains(privateKeyName)
    ...
    def "property sets commandline"() {
        ...
        "applicationLabel"        | "-a"            | "someApplicationLabel"
        "creator"                 | "-c"            | "someCreator"
        "description"             | "-D"            | "someDescription"
        "comment"                 | "-j"            | "someComment"
        "label"                   | "-l"            | "someLabel"
        "type"                    | "-t"            | FindKey.Type.Private
        "matchKeysThatCanDecrypt" | "-d"            | true
        "matchKeysThatCanEncrypt" | "-e"            | true
        "matchKeysThatCanSign"    | "-s"            | true
        "matchKeysThatCanDerive"  | "-r"            | true
        "matchKeysThatCanUnwrap"  | "-u"            | true
        "matchKeysThatCanVerify"  | "-v"            | true
        "matchKeysThatCanWrap"    | "-w"            | true
