...
import wooga.gradle.xcodebuild.XcodeSpec
import wooga.gradle.xcodebuild.config.BuildSettings

import static org.gradle.util.ConfigureUtil.configureUsing

trait XcodeTaskSpec implements XcodeSpec {

    private final ListProperty<String> additionalBuildArguments = objects.listProperty(String)
    ...
    private final Property<BuildSettings> buildSettings = objects.property(BuildSettings)
    ...
    void buildSettings(BuildSettings value) {
        buildSettings.set(value)
    }

    void buildSettings(Provider<BuildSettings> value) {
        buildSettings.set(value)
    }

    void buildSettings(Closure configuration) {
        buildSettings(configureUsing(configuration))
        this
    }

    void buildSettings(Action<BuildSettings> action) {
