...
import wooga.gradle.xcodebuild.XcodeBuildCommand
...
class XcodeArchive extends AbstractXcodeTask implements XcodeArchiveActionSpec {
    ...
    @Input
    final Provider<Set<XcodeBuildCommand>> buildActions
    ...
    XcodeArchive() {
        ...
        buildActions = project.provider({
            def s = new HashSet<XcodeBuildCommand>()
            s << XcodeBuildCommand.archive

            if (clean.present && clean.get()) {
                s << XcodeBuildCommand.clean
            }
            s
        })
        ...
        buildArguments = project.provider({
            ...
            if (projectOrWorkspace.name.endsWith(".xcworkspace")) {
                ...
                throw new ScriptException("xcode project path must be a valid .xcodeproj or .xcworkspace")
            }
